...
│   └── *.html          # 83 self-contained posts (own <style>, own :root, own nav markup)
...
├── images/             # 217 files — covers (<slug>-cover.jpg; books/ titles carry both language faces as -cover-en/-th or -cover-front/-back), 87 share cards (og-*.jpg, <slug>-og.jpg — all 1200×630), diagram and series-figure PNGs, posters, the event QR
...
### Content is organized as one featured post and six series
...
`blog/index.html` is therefore, in order: hero (its `.blog-jump` strip carries one chip per series,
kept honest by INV-02f) → **one featured post** → five `.series-section` blocks. No category-filter
UI, no client-side JS.
...
**Every post is bilingual.** All 83 carry the same pure-CSS TH ⇄ EN switch — a visually hidden
...
The hero `.blog-hero__stats` reads **6 Series · 83 Articles**; there is deliberately no
...
Current values, re-derive rather than trust: 6 Series · 83 Articles (7 Hermes Desktop + 20 AI Transformation + 10 Hermes + 13 OpenClaw + 24 DevOps + 9 Life) ·
