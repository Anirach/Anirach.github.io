...
│   └── *.html          # 76 self-contained posts (own <style>, own :root, own nav markup)
...
├── sitemap.xml         # 86 canonical URLs — GENERATED by scripts/gen_sitemap.py (2026-09-02;
│                       #   lastmod from git) — 404.html + PDFs excluded
├── favicon.ico         # + apple-touch-icon.png (180×180) — both probed at the root by default
├── images/             # 195 files — covers (<slug>-cover.jpg; books/ titles carry both language faces as -cover-en/-th or -cover-front/-back), 82 share cards (og-*.jpg, <slug>-og.jpg — all 1200×630), diagram and series-figure PNGs, posters, the event QR
...
### Content is organized as one featured post and five series
...
**Every post is bilingual.** All 76 carry the same pure-CSS TH ⇄ EN switch — a visually hidden
...
The hero `.blog-hero__stats` reads **5 Series · 76 Articles**; there is deliberately no
...
All 76 posts carry it. The mechanism is four CSS lines and no JavaScript:
...
Current values, re-derive rather than trust: 5 Series · 76 Articles (20 AI Transformation + 10 Hermes + 13 OpenClaw + 24 DevOps + 9 Life) ·
...
`books/three-old-men.html`) plus, since 2026-09-02, a BlogPosting block on all 76 posts
