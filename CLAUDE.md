...
│   └── *.html          # 93 self-contained posts (own <style>, own :root, own nav markup) — ALL posts live
│                       #   here, including the 9 Life essays that thoughts/ catalogues
├── thoughts/index.html # THOUGHTS catalog (2026-09-08) — the 9 Life Thought & Philosophy essays as one day in
│                       #   three parts (Morning · Noon · Twilight), plus the book they grow from. Hand-written;
│                       #   the series is complete. Cards link ../blog/<slug>.html — the posts never moved
...
`thoughts/index.html` is: Sunrise hero with a CSS half-sun (no counted stats) → a **companion band**
for *One Day of Light* (both cover faces, detail-page and PDF links — not a card) → one
...
**Every post is bilingual.** All 93 carry the same pure-CSS TH ⇄ EN switch — a visually hidden
...
|---|---|---|---|
| `.series-nav` chip strip | the 7 numbered OpenClaw posts, the 9 Life posts, the 10 Hermes posts, the 7 Hermes Desktop posts and the 10 AI-Core posts (each series its own strip) | `<div class="series-nav">` + `.series-links` with `<a>` for others and `<span class="current">` for self | absolute, extensionless: `/blog/openclaw-memory` |
...
Desktop + 20 AI Transformation + 10 Hermes + 13 OpenClaw + 24 DevOps) · 9 essays on `/thoughts/` ·
