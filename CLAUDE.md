...
Deploy by pushing to `main` — GitHub Pages auto-deploys. There is no `.nojekyll`, so Jekyll's default processing applies; GitHub Pages also resolves extensionless URLs (`/blog/openclaw-101` → `openclaw-101.html`), which the four chip-strip series rely on.
...
├── blog/
│   ├── index.html      # TUTORIALS catalog (nav label "Tutorials") — fully static, zero JavaScript: a featured
│   │                   #   post + six series tiles + the 6 technical series as numbered row cards (84 posts).
│   │                   #   GENERATED by scripts/reindex_blog.py — re-run it, do not hand-order
│   └── *.html          # 123 self-contained posts (own <style>, own :root, own nav markup) — ALL posts live
│                       #   here, including the 39 essays that thoughts/ catalogues
├── thoughts/index.html # THOUGHTS catalog (2026-09-08) — three series: the 20 Wisdom for a Good Life essays
│                       #   in four arcs of five (Self · Mind · Work · Others; launched 2026-09-09, GENERATED by
│                       #   build_series.py --series good-life, section assembled from the manifest), the 10
│                       #   Working Philosophies essays as one working week (Monday … Friday, two a day;
│                       #   GENERATED by build_series.py --series working) and the 9 Life Thought & Philosophy
│                       #   essays as one day in three parts (Morning · Noon · Twilight), plus the book they
│                       #   grow from. GENERATED since 2026-09-09 by reindex_blog.py --catalog thoughts
│                       #   in the SAME architecture as the blog catalog; cards link ../blog/<slug>.html —
│                       #   the posts never moved
...
├── robots.txt          # Allow all + explicit AI-crawler stanzas (13 bots, 2026-09-02)
│                       #   + Sitemap: line. VERIFIED 2026-08-26 post-deploy: the repo file
│                       #   REPLACES Cloudflare's Content Signals block outright — it is
│                       #   the site's ONLY AI-access policy surface
├── llms.txt            # AI-discoverability map (2026-09-02): site blurb + every post/
│                       #   book/section as absolute links — check_visibility.py L2 gates drift
├── sitemap.xml         # 93 canonical URLs — GENERATED by scripts/gen_sitemap.py (2026-09-02;
│                       #   lastmod from git) — 404.html + PDFs excluded
├── favicon.ico         # + apple-touch-icon.png (180×180) — both probed at the root by default
├── images/             # 431 files — covers (`<slug>-cover.jpg`; `books/` titles carry both language
│                       #   faces as -cover-en/-th or -cover-front/-back), 123 `<slug>-thumb.jpg` 144px card
│                       #   derivatives served via srcset (2026-09-10, a11y-perf R1b), 130 share cards
│                       #   (og-*.jpg, `<slug>-og.jpg` — all 1200×630), diagram and series-figure PNGs,
│                       #   posters, the event QR
...
**Every blog page is an island — and so is each of `books/` (its index and all four detail pages), `publications/`, `projects/`, `news/`, `thoughts/`.** None of them link `style.css`; each embeds its full stylesheet in a `<style>` block and defines its own `:root` variables (copied from the same canonical token set `style.css` also carries). Editing `style.css` affects only the landing page. Editing one post or one of the eight section-directory pages affects only that file — there is no shared partial, so changes that should apply "everywhere" must be repeated per file (this is what commit `4c180c9` "Standardize series navigation across all 7 OpenClaw posts" was doing).

### Content is organized as TWO catalog pages: Tutorials (`/blog/`, six series) and Thoughts (`/thoughts/`, three series)

**Split 2026-09-08** (spec: `docs/superpowers/specs/2026-09-08-thoughts-tutorials-split-design.md`).
The blog index used to be titled "Thoughts & Tutorials" and carried all seven series; the nine
Life essays sat at the foot of a 13,000px technical catalog. Now `/blog/` is the **Tutorials**
catalog (nav label "Tutorials" — "Blog" survives only in the URL and the feed title) and
`/thoughts/` is a new top-level section holding the Life series. **The post files did not move**:
every post is still `blog/<slug>.html`, and a card on `thoughts/index.html` links
`../blog/<slug>.html`. `check_site.py` calls both pages *catalogs* (discovered by their
`<section class="series-section">`), merges their cards, and requires every post to be carded
on exactly one of them (INV-01a/01c); INV-02a/b/c/f run per page, INV-02g polices the series
tiles, `gen_feed.py` reads both, and the nav on every chrome page and every post footer lists
`Tutorials · Thoughts` in place of `Blog`. The nine Life posts' `‹ Thoughts` / `← Back to
Thoughts` links point at `../thoughts/`; the 84 technical posts say `‹ Tutorials`.
...
`blog/index.html` is therefore, in order: hero (kept honest by INV-02f via the sticky `.blog-jump`
series bar; its sub-line links to Thoughts) → **one featured "Start here" post** → **"Choose a
series"**, six `.series-tile` anchors (icon · name · `N articles · ≈ reading time` · one line;
INV-02g checks count and time against the section, `class="series-tile"` never `"card"`) → six
`.series-section` blocks of numbered row cards in reading order. No category-filter UI, no
client-side JS.

`thoughts/index.html` mirrors the blog catalog since 2026-09-09 — **GENERATED by
`python3 scripts/reindex_blog.py --repo . --out thoughts/index.html --catalog thoughts`**, which
lifts the `<style>` from `blog/index.html` at run time so the two catalogs cannot drift apart. In
order: the same Sunrise hero (stats **3 Series · 39 Essays**) → a sticky 3-chip `.blog-jump` →
a "Start here" `.feature` spotlight (the newest essay's 1200×630 share card) → three
`.series-tile` anchors (INV-02g accepts `N essays` there) → three `.series-section` blocks of
numbered row cards with `.blog-grid__label` group rows (Good Life: Self · Mind · Work · Others;
Working: Monday…Friday; Life: Morning · Noon · Twilight) → the **companion band** for *One Day of
Light* between the Working and Life sections (both cover faces, detail-page and PDF links — not a
card). Card titles are `h3`, exactly as on the blog catalog.

- `#series-ai-transformation` — "AI Transformation for Organizations" (20 cards, numbered #1–#20;
  launched 2026-09-05) — making AI an organizational core rather than a pilot, a Deep Blue hero, a
  `--coral` accent on its covers and figures, a references block under each article, and a
  **grouped** 20-chip `.series-nav` strip: four labelled `.series-links__group` blocks of five
  (Reframe · Redesign · Engineer · Lead). It is the one series that is **generated** —
  `scripts/build_series.py` emits all 20 posts from a manifest plus per-post content sheets
  (see below)
- `#series-ai-core` — "Engineering AI-Core Systems" (10 posts, #1–#10; launched 2026-09-08) — a
  zero-to-hero how-to course rendered from the author's paper *Engineering AI-Core Systems: A
  Reference Architecture and Assurance Contract for Software 3.0*, every post a 7-step recipe
  advancing one running example (the KramKraft refund assistant), Deep Blue hero, green cover
  accent, a references block per post — the third **generated** series (`scripts/build_series.py
  --series ai-core`, manifest `scripts/series/ai-core.json`)
- `#series-hermes` — "Hermes Agent in Practice" (10 cards, all numbered #1–#10; launched 2026-09-01 as
  ...
  own 10-chip `.series-nav` strip, which INV-03c polices (no SERIES7 check does)
...
- `#series-good-life` — "Wisdom for a Good Life" (20 cards, #1–#20; launched 2026-09-09, carded on
  `thoughts/index.html`) — twenty essays each grown from two or three famous quotes, traced to
  their real sources (misattributions told plainly), weighed against the research with honest
  caveats, and closed with one practice for the week and a Sunday question. Four arcs of five —
  Self · Mind · Work · Others. Sunrise hero, navy/deep drawn covers with the **amber** accent, a
  **grouped** 20-chip `.series-nav` strip (four labelled groups of five, INV-03c/03d), a
...
- `#series-working` — "Working Philosophies" (10 cards, #1–#10; launched 2026-09-08, carded on
  ...
  fire), Thursday (the line, a fair fight), Friday (door and window, what you carry home). Sunrise
  hero, navy/deep drawn covers with the gold accent, a flat 10-chip `.series-nav` strip (INV-03c),
  every essay an epigraph → four sections → "Try this" practice → "A question to carry". The
  **fourth generated series**: `scripts/build_series.py --series working`, manifest
  `scripts/series/working.json`, which names `blog/morning-waking.html` as its skeleton (the
  Sunrise reference — the first series built off the Life family) and supplies the one rule that
  skeleton lacks through the manifest's `extra_css`
- `#series-life` — "Life Thought & Philosophy" (9 cards; completed 2026-09-01) — essays
  ...
  head-fake finale), with its own 9-chip `.series-nav` strip, likewise policed by INV-03c.
  **Carded on `thoughts/index.html`, not on the blog index**, since 2026-09-08

**Every post is bilingual.** All 123 carry the same pure-CSS TH ⇄ EN switch — a visually hidden
...
converted 2026-09-03 by `scripts/bilingualize.py`; the 20 AI Transformation posts were born
bilingual on 2026-09-05, emitted two-track by `scripts/build_series.py` — see "Bilingual posts"
below.

The hero `.blog-hero__stats` reads **6 Series · 84 Articles**; there is deliberately no
...
|---|---|---|---|
| `.series-nav` chip strip | the 7 numbered OpenClaw posts, the 9 Life posts, the 10 Working Philosophies posts, the 10 Hermes posts, the 7 Hermes Desktop posts and the 10 AI-Core posts (each series its own strip) | `<div class="series-nav">` + `.series-links` with `<a>` for others and `<span class="current">` for self | absolute, extensionless: `/blog/openclaw-memory` |
| `.series-nav` **grouped** chip strip | the 20 AI Transformation posts (2026-09-05) and the 20 Wisdom for a Good Life posts (2026-09-09, four labelled groups of five) | the same `<div class="series-nav">`, but `.series-links.series-links--grouped` holding four `.series-links__group` blocks of five chips, each led by a `<p class="series-links__label">`; INV-03d exists because the old `</div></div>` parser stopped at the first group | absolute, extensionless: `/blog/ai-transformation-prediction` |
...
All 83 posts carry it. The mechanism is four CSS lines and no JavaScript:
...
`scripts/build_series.py` is the same idea one level up: it emits a whole numbered series, already
two-track, from three inputs — the manifest `scripts/series/<name>.json` (the fixed strings:
numbers, slugs, chip labels, titles, hero subs, tags, read times, covers, figures), the per-post
content sheets `.bilingual/<slug>.{th,en}.html` (prose only, in bilingualize.py's delimiter
grammar), and a skeleton it reads at RUN TIME (`blog/hermes-101.html`), so the `<style>` block is
a real template rather than a copy that rots — `--check` diffs what it lifted. **The emitted posts
are the committed truth; the sheets in `.bilingual/` are scratch.** After launch only `--restrip`
touches a shipped file, and it rewrites nothing but the chip strip — that is the growth path for a
21st post: add it to the manifest, `--post <slug>` it, then `--restrip` the other twenty.

## Conventions

- **Language**: `<html lang="th">` on all 123 posts — Thai is the default track; the EN track is `lang="en"` wrappers behind the CSS switch, and the page-level `lang` never flips (it cannot, without JavaScript). All 7 nav-bearing index pages (`index.html`, `blog/index.html`, `thoughts/index.html`, `books/index.html`, `publications/index.html`, `projects/index.html`, `news/index.html`) and the 4 `books/` detail pages are `lang="en"`. Headings and technical terms in English, body prose in Thai (marked with `<span lang="th">` on the section pages).
...
- **Fonts**: one Google Fonts `<link>` per page. **Inter and JetBrains Mono are requested with
  the VARIABLE range syntax** — `Inter:wght@300..900`, `JetBrains+Mono:wght@400..600` — because the
  semicolon form (`wght@300;400;…;900`) makes the API serve one static file per weight: 7 files and
  331 KB for Inter's latin subset against 1 file and 47 KB. Switched sitewide 2026-09-10, saving
  **189 KB per page** (56% of font bytes). **Sarabun keeps the semicolon form** —
  `Sarabun:wght@400;500;600;700;800` — it has no variable version and a range request returns
  HTTP 400; all five static weights are used, and an unrequested one gets faux-bolded, which
  smears Thai glyphs. Two distinct URLs on disk: 115 pages with JetBrains Mono, 21 without.
  See a11y-perf R8b.
...
3. Add a `.card` anchor to the correct `.series-section` in `blog/index.html` (then re-run
   `python3 scripts/reindex_blog.py --repo . --out blog/index.html`) — or, for a Thoughts essay,
   to its section on `thoughts/index.html` with an `../blog/<slug>.html` href, then re-run the
   same script with `--catalog thoughts --out thoughts/index.html`.
4. Wire navigation: for the numbered OpenClaw series, add the chip to all 7 `.series-nav` blocks; for DevOps, insert into the prev/next chain by editing the two neighbouring posts as well; for AI Transformation, add the post to `scripts/series/ai-transformation.json` and run `python3 scripts/build_series.py --restrip` rather than hand-editing 20 grouped strips.
5. Update the counters (see below).

### Hand-typed counters — now enforced, but still hand-typed

`blog/index.html` carries three kinds: the hero's `N Series` / `N Articles`, a `.series-count` per
series, and each series tile's `N articles · ≈ reading time` (`N Categories` is retired with the
bands); `thoughts/index.html` carries one, its `.series-count` of `9 essays`. `reindex_blog.py`
computes the blog ones; nothing computes the thoughts one — but none of them is silent about
drift. `check_site.py` INV-02a/b/c/d/e/g recompute all of them from the actual card counts on
each catalog page and fail the build on any mismatch, and `--fix` rewrites the stats and
`.series-count` spans it can (tiles: re-run `reindex_blog.py`).
...
Current values, re-derive rather than trust: 6 Series · 84 Articles on `/blog/` (10 AI-Core + 7 Hermes
Desktop + 20 AI Transformation + 10 Hermes + 13 OpenClaw + 24 DevOps) · 39 essays on `/thoughts/` (20 Good Life + 10 Working Philosophies + 9 Life) ·
...
- **site-check** — run `python3 .claude/skills/site-check/scripts/check_site.py` from the repo root before every push, and `python3 scripts/check_visibility.py --strict` beside it (see "Search visibility"). 62 cross-file integrity checks (43 fail / 17 warn / 2 info); exit 0 means no fail-severity violation. The `BASELINE` table is **empty** since 2026-08-26 (`0 new, 0 known` — down from 55, then 29, paid down honestly in seven commits `01332eb`…`21d5cfc` rather than baselined; INV-05b was retired outright, 59 → 58), so every violation the script prints is new. `INV-25` fails the build if a baseline entry is ever added and goes stale, so the net cannot silently rot; `INV-26` ties every section-directory detail page (today: the four `books/*.html`) to its own index; `INV-27` enforces the social/canonical head block on all 86 pages. Added 2026-08-26 by the
...
exist together), `INV-31`/`INV-32` (feed and sitemap drift). Three more landed 2026-09-05 with the
AI Transformation series, all FAIL: `INV-03c` (every non-SERIES7 `.series-nav` strip is identical
across its members, marks exactly itself current, and links only to members — the Hermes, Life and
AI Transformation strips had no content check at all, because INV-03/03b/20 iterate the hardcoded
SERIES7 list), `INV-03d` (a linter self-check that the `.series-nav` parser survives a grouped
strip: `Site._parse_navs` now walks div depth via `series_nav_body()` instead of stopping at
RE_SNAV's first `</div></div>`), and `INV-02f` (the `.blog-jump` chips match the sections they
anchor — each chip anchors a real section, its trailing `· N` equals that section's card count,
one chip per section; nothing policed that strip, and the Life launch forgot its chip twice).
`INV-02g` (2026-09-08, FAIL) reads each `.series-tile` on a catalog page and checks its `N articles`
against the section's card count and its `≈ reading time` against the sum of the cards' `N min
read` — the tiles are generated, and this is what catches a hand-added post that skipped
`reindex_blog.py`. The same change generalised INV-01/02/07b/10/31/36 and `--fix` from "the blog
index" to "every catalog page" (`site.catalogs`, `site.card_page`, 4-tuple `site.sections`).
...
executes it; 79 pages use it — the three originals (`index.html`, `books/one-day-of-light.html`,
`books/three-old-men.html`) plus, since 2026-09-02, a BlogPosting block on all 83 posts
(the scope-broken microdata it replaced was stripped the same day), all schema.org metadata. INV-38 replaced INV-21/INV-21b, which
...
## Search visibility — scanner, llms.txt, robots AI policy (2026-09-02)

Learned from Houseofmvps/claude-rank (MIT) and adapted to this site:

- **`python3 scripts/check_visibility.py`** — the search/AI-visibility scanner. Report
  mode prints per-section 0–100 scores (claude-rank's deduction model); `--strict` exits
  1 on any FAIL-severity finding and belongs in the pre-push routine next to
  `check_site.py`; `--json` and `--min-score N` exist. It imports check_site.py for its
  parsers, so the two never drift. It deliberately does NOT adopt claude-rank's
  English-NLP content heuristics (broken on Thai prose), its file generators (they would
  clobber hand-maintained files), hreflang (the bilingual posts are single-URL — the
  signal is JSON-LD `inLanguage: ["th","en"]` + `og:locale:alternate`), or its 0–100
  score as a gate (check_site's per-violation empty-baseline model is stronger).
- **`llms.txt`** — the AI-discoverability map at the root: blurb + every post, book and
  section as absolute links. Regenerate the blog sections when posts change; the
  scanner's L2 check fails when a carded post is missing from it.
- **robots.txt welcomes AI crawlers explicitly** (13 stanzas). The wildcard already
  allowed everything; the stanzas make the policy legible — and they are load-bearing
  because this file REPLACES Cloudflare's Content Signals block.
- **Every post carries a JSON-LD BlogPosting block** in `<head>` right after
  `<!-- /social -->` (outside the delimiters, so social sweeps can't wipe it), built
  ...
  meta no longer carries `itemprop="image"`. Since every post is bilingual, all 76 carry
...
Added 2026-08-26. Every one of the 86 enumerated pages carries a delimited block in `<head>`:
...
**`sitemap.xml` is GENERATED** — `python3 scripts/gen_sitemap.py` (2026-09-02, reversing the
hand-maintained decision the way feed.xml's was reversed: on-demand, no build step, `lastmod`
from each file's last git commit, priorities per the standing convention, 404+PDFs excluded).
`--check` fails on drift; INV-32 remains the missing-page gate. Run it in the commit after a
content change so git dates exist.
