...
│                       #   grow from. The page is hand-written; cards link ../blog/<slug>.html — posts never moved
...
`thoughts/index.html` is: Sunrise hero with a CSS half-sun (no counted stats) → the
`#series-good-life` section (four `.part` blocks, Self · Mind · Work · Others, each an
`<h3 class="part-title">` and five cards; generated from `scripts/series/good-life.json` by a
scratch assembler — re-derive, don't hand-edit) → the `#series-working` section (five `.part` blocks, Monday to Friday, each an `<h3 class="part-title">`
and two cards in an `.essay-grid--pair`; generated from the manifest by a scratch assembler, so
re-derive it from `scripts/series/working.json` rather than hand-editing cards) → a **companion
band** for *One Day of Light* (both cover faces, detail-page and PDF links — not a card) → one
`#series-life` section (`<h2 class="series-title">`, `.series-count` **9 essays**) holding three
`.part` blocks — Morning · Noon · Twilight, each an `<h3 class="part-title">` and exactly three
cards in a 3-up grid showing the whole 800×800 cover uncropped, card titles at `h4`.
...
3. Add a `.card` anchor to the correct `.series-section` in `blog/index.html` (then re-run
   `python3 scripts/reindex_blog.py --repo . --out blog/index.html`) — or, for a Life essay, to the
   right `.part` on `thoughts/index.html` with an `../blog/<slug>.html` href.
