...
│   └── *.html          # 103 self-contained posts (own <style>, own :root, own nav markup) — ALL posts live
│                       #   here, including the 19 essays that thoughts/ catalogues
├── thoughts/index.html # THOUGHTS catalog (2026-09-08) — two series: the 10 Working Philosophies essays as one
│                       #   working week (Monday … Friday, two a day; GENERATED by build_series.py --series
│                       #   working) and the 9 Life Thought & Philosophy essays as one day in three parts
│                       #   (Morning · Noon · Twilight), plus the book they grow from. The page is hand-written;
│                       #   cards link ../blog/<slug>.html — the posts never moved
...
├── images/             # 247 files — covers (<slug>-cover.jpg; books/ titles carry both language faces as -cover-en/-th or -cover-front/-back), 99 share-card files (og-*.jpg, <slug>-og.jpg — all 1200×630), diagram and series-figure PNGs, posters, the event QR
...
### Content is organized as TWO catalog pages: Tutorials (`/blog/`, six series) and Thoughts (`/thoughts/`, one series)
...
`#series-working` section (five `.part` blocks, Monday to Friday, each an `<h3 class="part-title">`
...
**Every post is bilingual.** All 103 carry the same pure-CSS TH ⇄ EN switch — a visually hidden
...
| `.series-nav` **grouped** chip strip | the 20 AI Transformation posts (2026-09-05) | the same `<div class="series-nav">`, but `.series-links.series-links--grouped` holding four `.series-links__group` blocks of five chips, each led by a `<p class="series-links__label">`; INV-03d exists because the old `</div></div>` parser stopped at the first group | absolute, extensionless: `/blog/ai-transformation-prediction` |
...
Desktop + 20 AI Transformation + 10 Hermes + 13 OpenClaw + 24 DevOps) · 19 essays on `/thoughts/` (10 Working Philosophies + 9 Life) ·
