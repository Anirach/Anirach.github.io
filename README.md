...
| **Blog** | **123 posts** in 9 series across two catalogs, every one bilingual Thai/English |
...
| [`blog/`](blog/) | 123 bilingual posts in nine series — see below |
...
`blog/index.html` (Tutorials) is a static, zero-JavaScript catalog: one featured post, six series tiles, and six technical series as numbered row cards, with a chip strip in the hero that jumps to each; `thoughts/index.html` (Thoughts) holds the three essay series. There is no filtering, sorting or search UI.

- **🛡️ Engineering AI-Core Systems** — 10 posts, a zero-to-hero course from the author's own paper, generated by `scripts/build_series.py`.
- **💻 Hermes Desktop Hands-On** — 7 posts, generated, screen-by-screen guides.
- **🧭 AI Transformation for Organizations** — 20 posts, all numbered, linked by a chip strip grouped into four labelled blocks of five (Reframe · Redesign · Engineer · Lead). Generated: `scripts/build_series.py` emits all 20 from a manifest plus per-post content sheets.
...
- **🌱 Wisdom for a Good Life** — 20 essays on `/thoughts/`, each grown from two or three famous quotes traced to their real sources, in four arcs of five (Self · Mind · Work · Others). Generated.
- **🧭 Working Philosophies** — 10 essays on `/thoughts/`, one working week, Monday to Friday. Generated.
- **🌅 Life Thought & Philosophy** — 9 essays on `/thoughts/`, linked by a chip strip.
...
├── images/             # 308 files — covers, share cards, diagrams, series figures, posters, profile photo
